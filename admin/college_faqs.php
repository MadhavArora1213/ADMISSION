...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO college_faqs (id, college_id, question, answer, category, sort_order, is_active) 
                VALUES (UUID(), ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $college_id,
...
$stmt = $pdo->prepare("SELECT * FROM college_faqs WHERE college_id = ? ORDER BY sort_order ASC, question ASC");
$stmt->execute([$college_id]);
$faqs = $stmt->fetchAll();
?>
<!DOCTYPE html>
