...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO college_scholarships (id, college_id, scholarship_name, scholarship_type, amount, eligibility_criteria, renewable) 
                VALUES (UUID(), ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $college_id,
            ...
            header("Location: college_scholarships.php?college_id=$college_id&msg=added");
            exit;
        } catch (Exception $e) {
            $error = "Error adding scholarship: " . $e->getMessage();
        }
    } elseif (isset($_POST['action']) && $_POST['action'] == 'delete') {
        $stmt = $pdo->prepare("DELETE FROM college_scholarships WHERE id = ? AND college_id = ?");
        $stmt->execute([$_POST['s_id'], $college_id]);
        header("Location: college_scholarships.php?college_id=$college_id&msg=deleted");
        exit;
    }
}

$stmt = $pdo->prepare("SELECT * FROM college_scholarships WHERE college_id = ? ORDER BY scholarship_name ASC");
$stmt->execute([$college_id]);
