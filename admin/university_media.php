...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        ...
        $stmt = $pdo->prepare("UPDATE university_media SET virtual_tour_enabled = ? WHERE university_id = ? AND image_type IS NULL"); // Apply to identity row
        $stmt->execute([isset($_POST['virtual_tour_enabled']) ? 1 : 0, $university_id]);
...
$stmt = $pdo->prepare("SELECT * FROM university_media WHERE university_id = ? AND (image_url IS NOT NULL OR video_url IS NOT NULL OR document_url IS NOT NULL OR `360_tour_url` IS NOT NULL) ORDER BY sort_order ASC");
$stmt->execute([$university_id]);
$media = $stmt->fetchAll();

// Fetch virtual tour setting
$stmtTour = $pdo->prepare("SELECT virtual_tour_enabled FROM university_media WHERE university_id = ? AND image_type IS NULL LIMIT 1");
