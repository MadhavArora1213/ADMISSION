...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    $pdo->prepare("DELETE FROM visa_guides WHERE id = ?")->execute([$_GET['id']]);
    header("Location: visa_guides.php?msg=deleted");
    exit;
}
...
// Stats
$total_guides = $pdo->query("SELECT count(*) FROM visa_guides")->fetchColumn();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Visa Guides | AdmissionSeason Admin</title>
    ...
    <style>
        ...
        table { width: 100%; border-collapse: collapse; font-size: 0.85rem; min-width: 550px; }
    ...
    <main class="main-content">
        ...
        <div class="content-area">
            <div class="page-header">
                <div>
                    <h2><i class="ph ph-airplane-tilt" style="color:var(--primary);"></i> Visa Guides</h2>
                    <p style="color:var(--text-muted); margin-top:4px;">Manage visa information for various countries.</p>
                </div>
                <a href="visa_guide_form.php" class="btn-primary"><i class="ph ph-plus"></i> Add Guide</a>
            </div>
            
            <?php if(isset($_GET['msg']) && $_GET['msg'] == 'deleted'): ?>
            <div class="msg-alert"><i class="ph ph-check-circle"></i> Guide deleted successfully.</div>
            <?php endif; ?>
            
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="num"><?php echo $total_guides; ?></div>
                    <div class="label">Total Guides</div>
                </div>
            </div>
            ...
            <form class="filter-bar" method="GET">
                <div class="search-box">
                    ...
                    <input type="text" name="q" placeholder="Search Guides..." value="<?php echo htmlspecialchars($search); ?>">
            ...
            <div class="panel">
                <?php if(empty($guides)): ?>
                    <p style="color:var(--text-muted); text-align:center; padding: 40px;">No visa guides found.</p>
                <?php else: ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo $guide['processing_time_days'] ? htmlspecialchars($guide['processing_time_days']) . ' days' : '-'; ?></td>
                                <td><?php echo $guide['visa_fee_usd'] ? '$' . number_format($guide['visa_fee_usd'], 2) : '-'; ?></td>
                                <td>
                                    <div style="display:flex; gap:6px;">
                                        <a href="visa_guide_form.php?id=<?php echo $guide['id']; ?>" class="action-btn" title="Edit"><i class="ph ph-pencil-simple"></i></a>
                                        <a href="?action=delete&id=<?php echo $guide['id']; ?>" class="action-btn delete" title="Delete" onclick="return confirm('Delete this guide?');"><i class="ph ph-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
