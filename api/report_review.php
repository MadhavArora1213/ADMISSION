...
try {
    ...
    if ($existing->fetch()) {
        $upd = $pdo->prepare("UPDATE moderation_queue SET flagged_reason = CONCAT(flagged_reason, ', ', ?), priority = ? WHERE entity_type = 'review' AND entity_id = ? AND status IN ('pending','in_progress')");
        $upd->execute([$reasonText, $priority, $reviewId]);
    } else {
        ...
        $pdo->prepare("INSERT INTO moderation_queue (id, entity_type, entity_id, status, priority, flagged_reason, reporter_id, sla_due_at, created_at) VALUES (UUID(), 'review', ?, 'pending', ?, ?, ?, ?, NOW())")
             ->execute([$reviewId, $priority, $reasonText, $userId, $slaDue]);
    }
