...
if ($collegeId === '') {
    http_response_code(400);
    echo json_encode(['ok' => false, 'error' => 'college_id is required']);
    exit;
}
if ($overall < 1 || $overall > 5) {
    http_response_code(400);
    echo json_encode(['ok' => false, 'error' => 'Overall rating must be between 1 and 5']);
    exit;
}
if ($title === '' && $body === '') {
    http_response_code(400);
    echo json_encode(['ok' => false, 'error' => 'Please provide a review title or body.']);
    exit;
}
...
if (!$chk->fetch()) {
    ...
    echo json_encode(['ok' => false, 'error' => 'College not found']);
    exit;
}
...
if ($dup->fetch()) {
    ...
    echo json_encode(['ok' => false, 'error' => 'You have already reviewed this college.']);
    exit;
}
