...
$dup = $pdo->prepare("SELECT id FROM reviews WHERE user_id = ? AND school_id = ?");
$dup->execute([$userId, $schoolId]);
if ($dup->fetch()) {
    echo json_encode(['ok' => false, 'error' => 'You have already reviewed this school.']); exit;
}
...
$reviewId = uuid();
$stmt = $pdo->prepare("
    INSERT INTO reviews (id, user_id, school_id, overall_rating, academics_rating, faculty_rating, placements_rating, infrastructure_rating, social_life_rating, food_rating, review_title, review_body, pros, cons, batch_year, moderation_status, helpful_votes, reported_count, fraud_flag, created_at, updated_at)
...
try {
    ...
    $pdo->prepare("INSERT INTO moderation_queue (id, entity_type, entity_id, status, priority, flagged_reason, reporter_id, sla_due_at, created_at) VALUES (UUID(), 'school_review', ?, 'pending', ?, 'new_review', ?, ?, NOW())")
