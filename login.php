...
if (isset($_GET['ajax'])) {
    ...
    // Read redirect from POST body (sent via hidden field) instead of GET param
    // which gets lost during AJAX call since ?ajax= replaces ?redirect=
    $loginRedirect = $_POST['login_redirect'] ?? $loginRedirect;
    ...
    if ($action === 'verify_phone_email') {
        ...
        $form_mode = trim($_POST['form_mode'] ?? 'login');
...
$canonicalUrl = $siteBase . '/login.php';
$pageTitle = 'Login & Sign Up - Save Colleges, Apply & Get Counselling | AdmissionSeason';
$metaDesc = 'Login or sign up on AdmissionSeason to save favorite colleges, apply for admissions, get free counselling and track your application status. Join 5M+ students.';
$metaKeywords = 'AdmissionSeason login, sign up, college admission login, student login, apply college, save colleges, free counselling, admission application, student portal';
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <div class="form-section">
    ...
    <div id="auth_forms_wrapper">
      ...
      <form method="POST" action="" id="auth_form">
        ...
        <input type="hidden" name="login_redirect" id="login_redirect" value="<?= htmlspecialchars($loginRedirect) ?>">
