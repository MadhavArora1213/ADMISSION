...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($schoolId > 0 && $order >= 0 && $order <= 6) {
            if ($order > 0) {
                ...
                if ($dup) {
                    ...
                    header("Location: featured_schools.php?err=" . urlencode($error));
                    exit;
                }
            }
    ...
    if ($action === 'clear_all') {
        ...
        header("Location: featured_schools.php?msg=" . urlencode($msg));
        exit;
    }
}
                        ...
                        <table style="width:100%;border-collapse:collapse;font-size:.88rem">
                            ...
                            <tbody>
                                ...
                                <tr style="border-bottom:1px solid #f1f5f9">
                                    ...
                                    <td style="padding:10px 16px;color:var(--text-muted);font-size:.82rem"><?= htmlspecialchars($as['city_name'] ?? '') ?></td>
                                    <td style="padding:10px 16px;font-size:.82rem"><?= schoolTypeLabel($as['school_type']) ?></td>
                                    <td style="padding:10px 16px;font-size:.82rem"><?= !empty($as['overall_rating_avg']) && (float)$as['overall_rating_avg'] > 0 ? number_format((float)$as['overall_rating_avg'], 1) : '—' ?></td>
