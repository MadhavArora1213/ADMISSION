...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])) {
    $pdo->prepare("DELETE FROM subscriptions WHERE id = ?")->execute([$_GET['id']]);
...
<head>
    ...
    <title>Subscriptions | AdmissionSeason Admin</title>
    ...
    <main class="main-content">
        ...
        <div class="content-area">
            <div class="page-header">
                <div>
                    <h2><i class="ph ph-arrows-clockwise" style="color:var(--primary);"></i> Subscriptions</h2>
                    <p style="color:var(--text-muted); margin-top:4px;">Manage college subscriptions.</p>
                </div>
                <a href="subscription_form.php" class="btn-primary"><i class="ph ph-plus"></i> Add Subscription</a>
            </div>
            
            <?php if(isset($_GET['msg']) && $_GET['msg'] == 'deleted'): ?>
            <div class="msg-alert">Subscription deleted successfully.</div>
            <?php endif; ?>

            <div class="panel">
                <?php if(empty($subscriptions)): ?>
                    <p style="color:var(--text-muted); text-align:center; padding: 40px;">No subscriptions found.</p>
                <?php else: ?>
                <div style="overflow-x:auto;">
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><div style="font-weight:600;"><?php echo htmlspecialchars($sub['college_id']); ?></div></td>
                                <td><?php echo htmlspecialchars($sub['plan_name'] ?: 'Plan '.$sub['plan_id']); ?></td>
                                <td><span class="badge"><?php echo htmlspecialchars($sub['billing_cycle']); ?></span></td>
                                <td><?php echo date('Y-m-d', strtotime($sub['start_date'])) . ' to ' . date('Y-m-d', strtotime($sub['end_date'])); ?></td>
                                <td><span class="badge"><?php echo htmlspecialchars($sub['status']); ?></span></td>
                                <td>
                                    <div style="display:flex; gap:6px;">
                                        <a href="subscription_form.php?id=<?php echo $sub['id']; ?>" class="action-btn"><i class="ph ph-pencil-simple"></i></a>
                                        <a href="?action=delete&id=<?php echo $sub['id']; ?>" class="action-btn" onclick="return confirm('Delete?');"><i class="ph ph-trash"></i></a>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
